...
def generate(model, tokenizer, prompt: str, max_tokens: int = 256, gate_threshold: float = config.GATE_THRESHOLD) -> tuple[str, dict]:
    ...
    while tokens_generated < max_tokens:
        ...
        # Stop at the first gate that isn't confident — if we're unsure about
        # token t+2, we definitely can't trust t+3 (it would be conditioned
        # on a potentially wrong t+2).
